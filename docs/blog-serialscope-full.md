...
│  · 创建 BrowserWindow（主窗口 + 各模块独立窗口）           │
│  · 预加载脚本 preload.js 暴露受限 IPC                    │
│  · 负责 AI / MCP / 工作台执行授权                        │
└───────────────┬────────────────────────────────────────┘
                │ spawn + Named Pipe
                │ \\.\pipe\SerialScope.Native.<UUID>
                ▼
┌────────────────────────────────────────────────────────┐
│  C++ 独立后端 (backend/bin/serialscope-backend.exe)     │
│  · CSerialPort v5        串口枚举/打开/读写              │
│  · Boost.Asio            主事件循环                     │
│  · NamedPipeServer       单客户端 JSON-RPC 服务          │
│  · nlohmann/json         协议编解码                     │
│  · FrameDecoder          raw/delimiter/fixed/length 分帧  │
│  · ChecksumEngine        CRC8/16/32/SUM/XOR/LRC 校验    │
│  · AiAdapter             AI Provider 抽象门面（含 mock） │
└───────────────┬────────────────────────────────────────┘
                │ JSON-RPC 通知
                ▼
┌────────────────────────────────────────────────────────┐
│  Electron 渲染进程                                       │
│  ① src/renderer/renderer.js（原生 JS）                  │
│     串口调试 / 趋势 / 规则 / 宏 / 模拟器 / AI 规约解析    │
│  ② src/renderer-react/（React 19 + Vite + React Flow）   │
│     通信测试工作台（独立窗口渐进上线）                    │
└────────────────────────────────────────────────────────┘
```
...
# CSerialPort v5 仓库内锁定源码（替代 vcpkg 4.3.3 的版本）
```
...
```
backend.ping / backend.shutdown / backend.testPayload
ports.list
serial.status / serial.open / serial.close / serial.send
ai.status / ai.configure / ai.parseProtocol / ai.generateCommands
```
...
CSerialPort 内部线程
    → onReadEvent()
    → asio::post(io_, lambda)
    → Asio 主线程 handleReceived()
    → FrameDecoder 解码
    → 逐帧广播 serial:rx 事件
```
...
```
Raw（整块返回）
Delimiter（按 LF / CRLF / CR / 自定义 HEX 分隔）
Fixed（按 N 字节定长，上限 128 KiB）
Length-Field（按帧头 + 长度域动态帧长，含 lengthIncludesHeader / 大小端 / minFrameSize）
```
...
```
NONE / CRC8 / CRC16_MODBUS / CRC16_CCITT / CRC16_XMODEM / CRC32 / 
SUM8 / SUM16_LE / SUM16_BE / XOR / LRC
```
...
```
CRC8          -> 0x48
CRC16-MODBUS  -> 0x29B1（CRC 寄存器值，发送时低字节在前）
CRC16-CCITT   -> 0x29B1
CRC16-XMODEM  -> 0x31C3
CRC32         -> 0xCBF43926
```
...
```cpp
class AiProvider {
  bool requiresDataUpload() const = 0;
  std::string name() const = 0;
};
class AiAdapter {
  // 三层防线
  void ensureAuthorized(bool requireDataUpload);   // enabled + allowDataUpload
  void setProviderByName(std::string name);       // "mock" | "deepseek"
};
```
...
webPreferences: {
  ...
  sandbox: false,  // 仅工作台为 false（需 fs/child_process 调用）；其他窗口均为 true
  webSecurity: true
}
```
...
contextBridge.exposeInMainWorld('serialScope', {
  getBackendInfo, startBackend, callBackend,
  beginWorkbenchExecution, endWorkbenchExecution,
  launchSimulatorInstance, validateSimulatorAutoPort,
  reportSimulatorReady, reportSimulatorActivity, getSimulatorBootstrap,
  openModuleWindow, saveTextFile, openJsonFile, importProtocolFile,
  getAiConfig, configureAi, testAiConnection,
  startMcpServer, stopMcpServer, getMcpStatus, setMcpPorts,
  onBackendLog, onBackendExit, onBackendRpcNotification,
  onSimulatorOwnership, onSimulatorBootstrap,
  onSimulatorInstanceStatus, onUiAction
});
```
...
```js
if ((method === 'ai.parseProtocol' || method === 'ai.generateCommands')
    && ensureAiConfig().useDeepSeek()) {
  if (!ensureAiConfig().allowDataUpload) {
    throw new Error('[data-upload-denied] 允许数据上传后才能调用真实 provider');
  }
  ...
  if (method === 'ai.parseProtocol') {
    ...
    return { header: result.frame_format?.header, lengthFieldOffset: result.frame_format?.length_field?.offset, ... };
  }
...
return backendRpc.call(method, params);  // 否则回退 C++ Mock
```
...
```
runtimeApiKey (内存) ─→ savedApiKey (本地) ─→ env DEEPSEEK_API_KEY
```
...
```js
const COMMAND_SYSTEM_PROMPT = '...code 只含"从站地址+功能码+数据"，并把 checksum 标为 "modbus-crc16"；否则 checksum 填 "none"。校验码由程序本地计算追加...';
...
```html
<div class="dock">
  <section data-dock="rx">收发监视</section>
  <section data-dock="analyzer">帧分析助手</section>
  <section data-dock="rules">规则高亮</section>
  <section data-dock="send">发送区</section>
</div>
```
...
```js
// 自动查询的核心调度：单在途 token
async function scheduleAutoQuery(delayMs) {
  const token = ++autoQueryToken;
  setTimeout(async () => {
    if (token !== autoQueryToken) return; // 旧轮回调不能影响新轮
    await sendQuery(token);
    if (token !== autoQueryToken) return;
    scheduleAutoQuery(interval);
  }, delayMs);
}
```
...
版本             → AT+GMR (CRLF)
重启             → AT+RST (CRLF)
Modbus 读保持寄存器 → 01 03 00 00 00 02 (HEX, 自动追加 CRC16-Modbus)
Modbus 写单寄存器 → 01 06 00 01 00 64 (HEX, 自动追加 CRC16-Modbus)
心跳             → AA 55 00 01 (HEX)
```
...
```
匹配:    41 42                    # 收到 "AB"
回复:    AA 55 {{RAND8}} 0D 0A    # 帧头 + 1 字节随机 + CRLF
```
...
```
症状：DeepSeek 返回的 JSON.parse 报
  Expected ',' or ']' after array element in JSON at position 50 (line 4 column 15)
真因：模型返回的 code 数组用了 0x 十六进制字面量，如 [0x05, 0x1F, ...]，
      JSON 标准不支持 0x 前缀，JSON.parse 把 0 当数字，遇 x 就报错。
修复：deepseek-provider.js 增加 normalizeHexNumbers(jsonText)，
      把"字符串之外"的 0x[0-9a-fA-F]+ 归一化为十进制后再 JSON.parse。
      字符串字段内的 0x... 不被误改（如 description 里的说明文字）。
```
...
```
执行目标：
  ○ 模拟下位机回归（仅允许 COM10/COM11 虚拟端口）
  ○ 真实设备（需授权勾选 + beginWorkbenchExecution 二次确认）
```
...
过程检查通过：已验证 24 个活动 change；模板目录已跳过。
```

24 个变更包覆盖了：

```
add-ai-debug-log-and-key-save        # API Key 保存选项 + DeepSeek 调试日志
add-modbus-checksum-and-excel        # 命令校验码 + Excel 点表导入
add-deepseek-provider                # DeepSeek 真实 provider 接入（L3）
add-protocol-import                  # 规约文档导入（docx/pdf/txt/md/xlsx）
add-ai-command-generation            # AI 命令生成 + 宏库复用
add-ai-provider-adapter              # AI 适配层 + Mock（L2）
add-protocol-ai-parse                # 规约 AI 解析 + 校正 UI
add-device-test-workbench            # 通信测试工作台（React Flow）
add-standalone-windows-and-macro-editor
add-device-simulator                 # 设备模拟器
add-checksum-engine                  # C++ 校验引擎
add-length-field-framing             # 长度域分帧
add-mcp-server                       # MCP Server 桥接
...
```
...
npm run build:backend   # C++ 后端
npm run build:renderer  # React 工作台（Vite）
npm run dev             # 启动桌面应用（会先 build:renderer）
```
...
```powershell
npm run test:named-pipe          # 命名管道基础
npm run test:named-pipe-serial   # CSerialPort v5 虚拟串口
npm run test:named-pipe-fixed-frame
npm run test:named-pipe-outbound-boundary
npm run test:named-pipe-single-client
npm run test:electron-ui         # 可见 UI 自动化 + 虚拟串口回归
npm run test:ai-rpc              # AI Provider 边界
npm run test:deepseek            # DeepSeek 真实调用（需 DEEPSEEK_API_KEY 环境变量）
npm run test:protocol-import     # 规约文档导入
npm run test:mcp-handshake       # MCP 握手
npm run test:mcp-authorization   # MCP 授权边界
npm run test:protocol-ai-ui      # AI 规约解析 UI
npm run test:react-workbench-ui  # React 工作台 UI
npm run test:flow-runtime        # 工作台流程运行时
npm run test:device-workbench    # 工作台 + 设备联调
npm run test:workbench-dual-simulator
npm run process:check            # 变更包与证据校验
```
...
诚实地说，SerialScope 现在仍处于"功能闭环但很多细节待补"的阶段。下面这些不是"还没开始"，而是"已经在 backlog 上但需要按风险分档逐步推进"：

### 9.1 后端单元测试和协议测试

已有的 native tests 覆盖核心（CSerialPort、Named Pipe、FrameDecoder、ChecksumEngine、AiAdapter），但**协议级（serial.open/close/send/rx 全链路）的覆盖率还不完整**。这影响"自动化产线以 SerialScope 为后端"的可信度。

### 9.2 持久化日志与崩溃诊断

当前所有日志都是 `console` 级别，没有 ring buffer 落盘；崩溃时 Main / Renderer / C++ 后端的关联性难以还原。

### 9.3 自动重连策略

当前 serial.open 失败后，需要用户手动重试；没有指数退避 / 异常退避的"自动重连"。

### 9.4 大流量场景下的限流与缓冲

CSerialPort 默认 4 KiB 读缓冲，1 MiB FrameDecoder 环形缓冲——日常 9600/115200 场景足够；**接近 1 Mbps 极限速率或长帧（128 KiB+）时仍需压测**。

### 9.5 Windows 安装包、代码签名、自动更新

项目目前是"开发态运行"，没有 NSIS / MSI 打包、Authenticode 签名、auto-update 通道。

### 9.6 跨用户 / 跨 Windows 会话拒绝

Named Pipe 的 SID DACL 已通过单元测试，但**没有在不同 Windows 会话里实跑过**——AGENTS.md 的 G3 边界要求"必须真实跑通"才算完成。

### 9.7 CSerialPort 的 LGPL 许可证审查
